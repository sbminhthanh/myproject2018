					...
					<tbody>
                        ...
                        <tr role="row" class="odd">
                            <td class="input_check">
								<div class="ichack-input">
									<input type="checkbox" class="minimal" id="id_{{$valItem->id}}" name="id[]" value="{{$valItem->id}}">
								</div>
							</td>
							<td class="item_index">
								{{$keyItem+1}} 
							</td>
							<td class="sorting_1">{{$valItem->category_id}}</td>
							<td>
								<?= str_repeat('|____',$valItem->depth) ?>{{$valItem->category_name}}
							</td>							
							<td class="item_check">
								@if($valItem->sticky==1)
                            ...
                            <td class="action_tablel">           
								<a href="{{$__URL['update']}}/{{$valItem->id}}">
									<span class="label bg-aqua">
										Thay đổi
										<i class="fa fa-pencil"></i>
									</span> 
								</a>
								<a href="{{$__URL['delete']}}/{{$valItem->id}}">
