...
<div class="col-12">	
	<div class="box box-default">
          ...
          <!-- /.box-header -->
          <div class="box-body">
            <form method="post" action="admin/product/category/edit/{{$product_category->id}}">
             {{ csrf_field() }}             
              ...
              <div class="row">
                <div class="col-12">
                  <div class="form-group row">
                    ...
                    <div class="col-sm-8">
                      <input class="form-control" type="text" value="@if(!empty(old('id'))){{old('id')}}@else{{$product_category->category_id}}@endif" name="id" placeholder="Mã danh mục">
                    </div>
                    <div class="col-sm-2">
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                      <input class="form-control" type="text" id="name" placeholder="Tên danh mục" value="@if(!empty(old('name'))){{old('name')}}@else{{$product_category->category_name}}@endif" name="name">
                    </div>
                  </div>
                      ...
                      <select class="form-control select2" name="parent_id" style="width: 100%;">
                        ...
                        @foreach($select_Product_Category as $key => $value)
                          <option value="{{$value->id}}" 
                            ...
                            @else
                              @if($value->id == $product_category->parent_id){{"selected"}}@endif
                            @endif
                            @if($value->id == $product_category->id){{ "disabled" }}@endif
                          >
                            <?= str_repeat('|==',$value->depth) ?>{{$value->category_name}}
                          </option>
                        @endforeach
                      </select>
                    </div>
                  ...
                  @if($status_upload)
                  <div id="form_file">
                        <?php $List_image =  json_decode($product_category->images, JSON_UNESCAPED_UNICODE); ?>
                  ...
                  <div id="tab_element" class="tab_editor">
                    ...
                    <div class="resp-tabs-container tab_element">
                        <div>
                            <textarea name="description" id="description" class="ckeditor_textarea form-control" >@if(!empty(old('description'))){{old('description')}}@else{{$product_category->category_description}}@endif</textarea>
                        </div>
                        <div>
                            <textarea name="content" id="content" class="ckeditor_textarea form-control" >@if(!empty(old('content'))){{old('content')}}@else{{$product_category->category_content}}@endif</textarea>   
                        </div>                        
                    </div>
                  </div>
                 {{--  SEO  --}}
                  <div class="box-header ">
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-sm-10">
                        <input class="form-control" type="text" id="seo_title" placeholder="SEO title" value="@if(!empty(old('seo_title'))){{old('seo_title')}}@else{{$product_category->seo_title}}@endif" name="seo_title">
                      </div>
                    </div>
                    <div class="form-group row">
                      <label for="seo_slug" class="col-sm-2 col-form-label">Slug:</label>
                      <div class="col-sm-10">
                        <input class="form-control" type="text" id="seo_slug" placeholder="Slug" value="@if(!empty(old('seo_slug'))){{old('seo_slug')}}@else{{$product_category->slug}}@endif" name="seo_slug">
                      </div>
                    </div>
                    <div class="form-group row">
                      <label for="meta_description" class="col-sm-2 col-form-label">Meta Description:</label>
                      <div class="col-sm-10">
                        <textarea name="meta_description" id="meta_description" class="form-control" placeholder="Meta Description" >@if(!empty(old('meta_description'))){{old('meta_description')}}@else{{$product_category->meta_description}}@endif</textarea>   
                      </div>
                    </div>
                    <div class="form-group row">
                      <label for="focus_keyword" class="col-sm-2 col-form-label">Focus Keyword:</label>
                      <div class="col-sm-10">
                        <input class="form-control" type="text" id="focus_keyword" placeholder="Focus Keyword" value="@if(!empty(old('focus_keyword'))){{old('focus_keyword')}}@else{{$product_category->focus_keyword}}@endif" name="focus_keyword">
                      </div>
                    </div>
                    <div class="form-group row">
                      <label for="meta_keyword" class="col-sm-2 col-form-label">Meta Keyword:</label>
                      <div class="col-sm-10">
                        <input class="form-control" type="text" id="meta_keyword" placeholder="Meta Keyword" value="@if(!empty(old('meta_keyword'))){{old('meta_keyword')}}@else{{$product_category->meta_keyword}}@endif" name="meta_keyword">
                      </div>
                    </div>
                    <div class="form-group row">
                      <label for="meta_title" class="col-sm-2 col-form-label">Meta Title:</label>
                      <div class="col-sm-10">
                        <input class="form-control" type="text" id="meta_title" placeholder="Meta Title" value="@if(!empty(old('meta_title'))){{old('meta_title')}}@else{{$product_category->meta_title}}@endif" name="meta_title">
                      </div>
                    </div>                    
                  </div>
                 {{--  Button  --}}
                    <div class="form-group row">
                      <div class="col-sm-12" style="text-align:right">
                        <a  href="admin/product/category/edit/{{$product_category->id}}" class="btn btn-app bg-red">
                          <i class="fa fa-refresh"></i> Hoàn tác
                        </a> 
                        <button type="submit" class="btn btn-app bg-aqua">
                          <i class="fa fa-save"></i> Lưu
                        </button>
                        <a href="admin/product/category/list" class="btn btn-app bg-yellow">
