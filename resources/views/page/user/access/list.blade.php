	...
	<div class="box">
		...
		<div class="box-body">
			<div class="box_button_table">	
				<a class="btn btn-app bg-blue" href="admin/user/access/add">
			...
			<div class="dataTables_wrapper">			
				<table id="table_responsive"  class="table table-bordered table-hover display nowrap margin-top-10 table-responsive dataTable" role="grid" >
					...
					<tbody>
						@foreach($List_Access as $key => $item)
                        <tr role="row" class="odd">
							...
							<td class="sorting_1">{{$item->access_id}}</td>
							<td>
								{{$item->access_name}}
							</td>							
							<td>
								{{strip_tags($item->access_description)}}
							</td>	
							<td class="item_check">
								@if($item->status==1)
                            ...
                            <td class="action_tablel">           
								<a href="admin/user/access/edit/{{$item->id}}">
									<span class="label bg-aqua">
										Thay đổi
										<i class="fa fa-pencil"></i>
									</span> 
								</a>
								<a href="admin/user/access/delete/{{$item->id}}">
...
<?php
$excel = Config::get('constants.file.excel.product.category');
?>
	<!-- DataTables -->
