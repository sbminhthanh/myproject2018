...
<?php
  ...
  $access_edit = convert_Array_Json_Access($access_edit,$access->product,"product");
  $access_edit = convert_Array_Json_Access($access_edit,$access->news,"news");
  $access_edit = convert_Array_Json_Access($access_edit,$access->account,"account");
  $access_edit = convert_Array_Json_Access($access_edit,$access->gallery,"gallery");
  $access_edit = convert_Array_Json_Access($access_edit,$access->cart,"cart");
  $access_edit = convert_Array_Json_Access($access_edit,$access->config,"config");
?>
<div class="col-12">	
	<div class="box box-default">
          ...
          <!-- /.box-header -->
          <div class="box-body">
            <form method="post" action="admin/user/access/edit/{{$access->id}}">
             {{ csrf_field() }}
              ...
              <div class="row">
                <div class="col-12">
                  <div class="form-group row">
                    ...
                    <div class="col-sm-8">
                      <input class="form-control" type="text" value="@if(empty(old('id'))){{$access->access_id}}@else{{old('id')}}@endif" name="id" placeholder="Mã">
                    </div>
                    <div class="col-sm-2">                    
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                      <input class="form-control" type="text" id="name" placeholder="Tên quyền hạn cho tài khoản" value="@if(empty(old('name'))){{$access->access_id}}@else{{old('name')}}@endif" name="name">
                    </div>
                  </div>
                  <hr/>
                  {{-- Quyền --}}
                  ...
                  @if($status_upload)
                  <div id="form_file">
                        <?php $List_image =  json_decode($access->images, JSON_UNESCAPED_UNICODE); ?>
                  ...
                  <div id="tab_element" class="tab_editor">
                    ...
                    <div class="resp-tabs-container tab_element">
                        <div>
                            <textarea name="description" id="description" class="ckeditor_textarea form-control" >@if(!empty(old('description'))){{old('description')}}@else{{$access->access_description}}@endif</textarea>
                        </div>
                        <div>
                            <textarea name="content" id="content" class="ckeditor_textarea form-control" >@if(!empty(old('content'))){{old('content')}}@else{{$access->access_content}}@endif</textarea>   
                        </div>                        
                    </div>
                  </div>
